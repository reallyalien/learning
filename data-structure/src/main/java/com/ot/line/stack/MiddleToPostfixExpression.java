...
/**
 * 中缀转后缀需要一个栈和一个数组，栈用来处理符号，队列用来存储操作数和处理过的符号
 */
            ...
            if (s.matches("\\d")) { //如果是数字
                ...
                if (s1.isEmpty()) {
                    ...
                    if (leftOrRightBrackets(peek) == 1) {
                        ...
                        //因为在中缀表达式转后缀表达式当中，在后缀表达式当中一直是弹出符号栈然后计算，因此优先级高的必须在上面。
                ...
                if (leftOrRightBrackets(s) == 1) {//左括号直接入栈
                    ...
                    while (leftOrRightBrackets(s1.peek()) != 1) { //这里只找左括号，是普通符号也没有关系
                        String pop = s1.pop();
                        s2.add(pop);
                    }
                    //如果碰到右括号，必然之前存在左括号，上面的while循环停止，接下来的一个就是左括号，弹出，丢弃这个左括号
                    s1.pop();
                }
            }
        }
