...
/**
 * 终极：根本没有异步阻塞和异步非阻塞这一说，完全是扯淡，只有同步才会有这一说，
 * 同步和异步指的是：应用程序之间的协调状态
 * 而阻塞和非阻塞更多的是指单个应用程序的状态
 * 异步需要cpu的参与：只有用户线程根本不去考虑io的全部执行全部交给cpu来完成，而自己只等一个结果才是异步io，所以拉一个子线程去轮询或者
 * select，poll，epoll都不是异步
 */
public class Server {
    //定义属性
